...
def validate_spreadsheet_file(value: File) -> None:
    ...
    try:
        validator: FileExtensionValidator = FileExtensionValidator(
...
def validate_email_unique(value: str) -> None:
    ...
    filters: Q = Q(username__iexact=value) | Q(email__iexact=value)
    queryset: QuerySet = get_user_model().objects.filter(filters)
